...
CMD = 'squeue -o "%a %u %i %j %T %M %l %C %D %q %P %p %R"'
CMDSACCT  = 'sacct -j %s --format JobId,State,Start,End -X -n'
...
def subjobs(jobs, field, val, inverse=False, test=None):
    ...
    'PARTITION', 'PRIORITY', 'QOS', 'STATE', 'TIME', 'TIME_LIMIT', 'USER'
    """
    if not test:
        same = lambda x, y: x == y
        diff = lambda x, y: x != y
...
def print_stats(jobs, users, width=86, verbose=2, # jobsname=None,
    ...
    :param 86 width: print area width
    ...
    for user in users + (['total'] if len(users) else []):
        ...
        if groupby == 'TIME_LIMIT' and not user=='total':
            ...
            groups = [None]
        for grp in groups:
            if groupby == 'TIME_LIMIT' and user!='total':
                grpjobs = subjobs(userjobs, 'TIME_LIMIT', grp,
...
def single_stats(running, pending, dependy, cmpling, done, error, limtime,
    ...
    if verbose == 2:
        ...
        sprio = ''
    def get_bar(prio):
        for s in '|' * (running + pending + dependy + cmpling + done + error -
                        len(prio) - 1) + prio + '|':
            yield s
    priobar = get_bar(sprio)
    out = ''
    if verbose == 2:
        out += ('run:\033[0;31m%-4s\033[m ' +
                ...
                'T:%-7s(.-%-7s)' + 
                '<%s\n') % tuple(map(frm_num, [running, cmpling, pending,
                                               dependy, done + error, error,
                                               runtime, maxtime, limtime]))
    else:
        out += ('run:\033[0;31m%-4s\033[m ' +
                'cpl:\033[0;35m%-1s\033[m ' + 
                'pen:\033[0;33m%-5s\033[m' + 
                ...
                'T:%-7s<%-7s'+
                ' prio:%s\n') % tuple(map(
            frm_num, [running, cmpling, pending, dependy, done + error, error,
                      runtime, limtime, round(avgprio[2] * 1000, 3)]))        
    if verbose:
        if grp > -1:
            out += '%-12s' % (('  ~' + time2str(int(TIME_ROUND * (grp + 1.5)),
                                                tround='m'))
                              if groupby == 'TIME_LIMIT' else grp
                              if groupby == 'NAME' else '')
        else:
            out += ' ' * 12
...
def time2str(seconds, tround='s'):
    ...
    seconds -= minutes * 60
    if tround == 'm':
        return '%2s-%02d:%02d' % (days, hours, minutes)
    return '%2s-%02d:%02d:%02d' % (days, hours, minutes, seconds)


def get_time(jobs, kind='TIME_LIMIT', tround='s', calc='avg', fact=1):
    if kind == 'SPENT':
...
******
  ...
  * g: group by a given element (implemented: TIME_LIMIT)
  * h: help
  ...
  * q: quit
  * r: reload
  * w: change width of the display (min 86)
    ...
    while True:
        ...
        if s == 'r':
            ...
            print '\n   %-8s %-30s %-10s %-4s %-10s %-10s' % ('JOBID', 'JOBNAME',
                                                              ...
                                                              'TIME', 'TIME_LIMIT')
            print '   ' + '-' * 76
            for j in sorted(jobs, key=lambda x: totime(jobs[x]['TIME'])):
                ...
                print '   %-8s %-30s %-10s %-4s %-10s %-10s' % (
                    ...
                    jobs[j]['CPUS'], jobs[j]['TIME'], jobs[j]['TIME_LIMIT'])
        elif s == 'j':
            print '\n   %-8s %-30s %-10s %-4s %-10s %-10s' % ('JOBID', 'JOBNAME',
                                                              'STATE', 'CPUS',
                                                              'TIME', 'TIME_LIMIT')
            ...
            width = raw_input('new width (actual=%s; minimum=86; "auto" for max available): ' % opts.width)
            try:
                width = int(width)
                AUTOWIDTH = False
            except ValueError:
                if width == 'auto':
                    ...
                    toreload  = False
                    break
            opts.width = 86 if width < 86 else width
            toreload  = False
            break
        elif s == 'g':
            blanks = ' ' * 15
...
def get_options():
    ...
    parser.add_option('--groupby', action='store', default=None,
                      ...
                      help=('[%default] group user jobs by time limit ' +
                            '(groupby=time).'))
    parser.add_option('--width', action='store', default="auto (%s)" % (width - 4),
                      dest='width',
                      help='[%default] display width (should be > 86)')
    ...
    opts.groupby = ('TIME_LIMIT' if opts.groupby == 'time' else 'NAME'
                    if opts.groupby == 'name' else None)
    return opts
